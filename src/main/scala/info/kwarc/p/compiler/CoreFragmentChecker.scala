...
object CoreFragmentChecker extends Traverser[CoreFragmentContext] {
  ...
  override def apply(
      exp: Expression
  )(implicit gc: GlobalContext, ctx: CoreFragmentContext): Expression = {
  ...
  override def apply(
      thy: Theory
  )(implicit gc: GlobalContext, ctx: CoreFragmentContext): Theory = {
    val nCtx = ctx.copy(declared = false)
    matchC(thy) {
      case TheoryValue(decls) =>
        val includedNames = decls.flatMap {
        ...
        decls.foreach {
          case ExprDecl(declName, _, _, _, _, _) =>
            if (!includedNames.contains(declName))
  ...
  override def apply(
      tp: Type
  )(implicit gc: GlobalContext, ctx: CoreFragmentContext): Type = {
    val nCtx = ctx.copy(declared = false)
    matchC(tp) {
  ...
  override def apply(
      decl: Declaration
  )(implicit gc: GlobalContext, ctx: CoreFragmentContext): Declaration = {
