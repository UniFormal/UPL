module Slingshot{
  type point
  dist: point -> point -> float
  grav_acc: float = 9.81
  
  theory Slingshot_schema{
    launch: point
    launch_base: point
    h_launch: float
    h_launch_P: |- h_launch == dist(launch_base)(launch)
    v_z: float // vertical launch velocity (along the z-axis)
    v_h: float // horizontal launch velocity (x- and y-axis are indiscernible)
    v_launch: float
    v_launch_Ax: |- (v_z^2 + v_h^2) == v_launch^2
    inclination_launch: float
    inclination_Ax: |- inclination_launch == (v_z / v_h)

    target: point
    dist_target: float 
    dist_target_P: |- dist_target == dist(launch_base)(target)

    // Just the quadratic formula for b = v_z, c = h_launch and a = -1/2*grav_acc
    // both (-1/2) are canceled implicitly
    impact_time: float = (v_z + CAST(float,(v_z^2 + 2*h_launch*grav_acc)^(1/2)))/grav_acc
    // There is no horizontal acceleration or offset (Yes, we ignore air-resistance)
    // impact_distance: float = v_h * impact_time
    // impact_distance_Ax: |- impact_distance == dist_target 

    // We don't actually care about the impact_distance, only that it is on target => just substitute it immediately
    impact_at_target_Ax: |- v_h * impact_time == dist_target
  }

  // launch with no height offset
  theory Slingshot_groundLaunch{
    launch: point
    launch_base: point = launch
    h_launch: float = 0
    h_launch_P: |- dist(launch_base)(launch) == h_launch
    v_z: float // vertical launch velocity (along the z-axis)
    v_h: float // horizontal launch velocity (x- and y-axis are indiscernible)
    v_launch: float
    v_launch_Ax: |- (v_z^2 + v_h^2) == v_launch^2
    inclination_launch: float
    inclination_Ax: |- inclination_launch == (v_z / v_h)

    target: point
    dist_target: float
    dist_target_P: |- dist(launch_base)(target) == dist_target

    // Just the quadratic formula for b = v_z, c = h_launch and a = -1/2*grav_acc
    // both (-1/2) are canceled implicitly
    impact_time: float
    impact_time_Ax: |- (v_z + ((v_z*v_z) + 2*h_launch*grav_acc)^(1/2))/grav_acc == impact_time

    // There is no horizontal acceleration or offset (Yes, we ignore air-resistance)
    // impact_distance: float = v_h * impact_time
    // impact_distance_Ax: |- impact_distance == dist_target

    // We don't actually care about the impact_distance, only that it is on target => just substitute it immediately
    impact_at_target_Ax: |- v_h * impact_time == dist_target
    
    // //
    // Simplifications from our assumptions:
    // //

    // The quadratic formula for b = v_z, c = 0 and a = -1/2*grav_acc, with implicit cancelling:
    // sqrt(v_z^2 - 4*0*..) ~> v_z
    // -v_z +- v_z ~> -v_z - v_z (We don't care about the root at 0)
    // 2*(-1/2*grav_acc) ~> -grav_acc
    // (-v_z - v_z) / (-grav_acc) ~> 2*v_z / grav_acc
    impact_time_Ax_Fixed: |- 2*v_z / grav_acc == impact_time

    // equate `impact_at_target_Ax` and `impact_time_Ax_Fixed` (the two calculations of `impact_time`), 
    // rearrange and use `inclination_Ax`
    // equated: |- dist_target * grav_acc == 2*v_z*v_h
    impact_at_time_Fixed: |- dist_target * grav_acc  == 2 * inclination_launch * v_h^2
    impact_at_time_Fixed2: |- dist_target * grav_acc / 2 / inclination_launch == v_h^2
  }

  // also fix the launch angle to a convenient value
  theory Slingshot_fixedAngle_groundLaunch{
    launch: point
    launch_base: point = launch
    h_launch: float = 0
    h_launch_P: |- dist(launch_base)(launch) == h_launch
    v_z: float // vertical launch velocity (along the z-axis)
    v_h: float // horizontal launch velocity (x- and y-axis are indiscernible)
    v_launch: float
    inclination_launch = 0.75 // == 3/4
    // about that inclination:
    // $sqrt(x^2+(ax)^2) = sqrt((a^2+1) * x^2) = sqrt(a^2+1)*x$ (for x>0)
    // we want $a$ s.t. $sqrt(a^2+1) = n/m$ rational, and  $(n/m)^2 +1 = (n^2/m^2) + (m^2/m^2) = (n^2+m^2)/m^2$ 
    // so, we want $n^2+m^2 = k^2$, i.e. a [Pythagorean triple](https://en.wikipedia.org/wiki/Pythagorean_triple)
    // We pick the simplest one (3,4,5), because 3/4 and 5/4 both have exact float representations.
    // We chose v_z as basis, because it occurs more often. 
    // (The inclination is defined v_z/v_h though, so *shrug*)
    inclination_Ax: |- inclination_launch == (v_z / v_h)

    v_launch_Ax: |- (v_z^2 + v_h^2) == v_launch^2

    target: point
    dist_target: float
    dist_target_P: |- dist(launch_base)(target) == dist_target


    impact_time: float
    impact_time_P: |- (v_z + CAST(float,(v_z^2 + 2*h_launch*grav_acc)^(1/2)))/grav_acc ==  impact_time

    // There is no horizontal acceleration or offset (Yes, we ignore air-resistance)
    // impact_distance: float = v_h * impact_time
    // impact_distance_Ax: |- impact_distance == dist_target

    // We don't actually care about the impact_distance, only that it is on target => just substitute it immediately
    impact_at_target_Ax: |- v_h * impact_time == dist_target
    
    // //
    // Simplifications from our assumptions:
    // //
    inclination_Ax_Fixed: |- v_h * 0.75 == v_z
    inclination_Ax_Fixed2: |- v_z / 0.75 == v_h

    // sqrt(x^2+(3/4x)^2) == sqrt((4^2+3^2)/(4^2) * x^2) == 5/4x = 1.25x, see above
    v_launch_Ax_Fixed: |- 1.25 * v_h == v_launch

    // The quadratic formula for b = v_z, c = 0 and a = -1/2*grav_acc, with implicit cancelling:
    // sqrt(v_z^2 - 4*0*..) ~> v_z
    // -v_z +- v_z ~> -v_z - v_z (We don't care about the root at 0)
    // 2*(-1/2*grav_acc) ~> -grav_acc
    // (-v_z - v_z) / (-grav_acc) ~> 2*v_z / grav_acc
    impact_time_P_Fixed: |- 2*v_z / grav_acc == impact_time

    // equate the two calculations of `impact_time`, and rearrange
    impact_at_time_Fixed: |- dist_target * grav_acc == 2*v_z*v_h
    impact_at_time_Fixed2: |- dist_target * grav_acc == 2*0.75* v_h^2

    // plugging `inclination_Ax_Fixed2` into `impact_at_target_Ax`
    impact_at_target_Ax_Fixed2: |- (v_z / 0.75) * impact_time == dist_target
    // reorder and equate with `impact_time_P_Fixed`
    equated1: |- 2*v_z / grav_acc == dist_target * 0.75 / v_z 
    equated2: |- 2*v_z*v_z == dist_target * 0.75 * grav_acc
  }

  // no "correctness fluff"
  theory Slingshot_simple{
    h_launch: float
    v_z: float // vertical launch velocity (along the z-axis)
    v_h: float // horizontal launch velocity (x- and y-axis are indiscernible)
    v_launch: float
    inclination_launch = 8/15
    // about that inclination:
    // $sqrt(x^2+(ax)^2) = sqrt((a^2+1) * x^2) = sqrt(a^2+1)*x$ (for x>0)
    // we want $a$ s.t. $sqrt(a^2+1) = n/m$ rational, and  $(n/m)^2 +1 = (n^2/m^2) + (m^2/m^2) = (n^2+m^2)/m^2$ 
    // so, we want $n^2+m^2 = k^2$, i.e. a [Pythagorean triple](https://en.wikipedia.org/wiki/Pythagorean_triple)
    // looking through them, we find (8,15,17), which is nice, because 15/8 and 17/8 both have exact float representations.
    // We chose v_z as basis, because it occurs more often. 
    // (The inclination is defined v_z/v_h though, so *shrug*)
    inclination_Ax: |- inclination_launch == (v_z / v_h)
    // sqrt(x^2+(15/8x)^2) == sqrt((15^2+8^2)/(8^2) * x^2) == 17/8x = 2.125x, see above
    v_launch_Ax: |- v_launch == 2.125 * v_z 

    dist_target: float

    // Just the quadratic formula for b = v_z, c = h_launch and a = -1/2*grav_acc
    // the (-1/2) part is canceled implicitly
    impact_time: float
    impact_time_P: |- (v_z + CAST(float,(v_z^2 + 2*h_launch*grav_acc)^(1/2)))/grav_acc ==  impact_time
    // There is no horizontal acceleration or offset (Yes, we ignore air-resistance)
    // impact_distance: float = v_h * impact_time
    // impact_distance_Ax: |- impact_distance == dist_target

    // We don't actually care about the impact_distance, only that it is on target => just substitute it immediately
    impact_at_target_Ax: |- v_h * impact_time == dist_target
  }

  theory SlingshotScroll{
    launch: point
    v_z: float // vertical launch velocity (z-axis)
    v_h: float // horizontal launch velocity 
    v_launch: float // total launch velocity
    v_launch_Ax: |- (v_z^2 + v_h^2) == v_launch^2
    inclination_launch: float
    inclination_Ax: |- inclination_launch == (v_z / v_h)

    target: point
    dist_target: float
    dist_target_prop: |- dist(launch)(target) == dist_target

    // Quadratic formula for b = v_z, c = 0 and a = -1/2*grav_acc
    // the (-1/2) and 0 are already canceled for brevity
    impact_time: float
    impact_time_P: |- (v_z+(v_z^2)^(1/2))/grav_acc ==  impact_time

    // There is no horizontal acceleration or offset
    impact_at_target_Ax: |- v_h * impact_time == dist_target

    // equate the two calculations of `impact_time`, 
    // rearrange and use `inclination_Ax`
    impact_at_time: |- dist_target * grav_acc  == 2 * inclination_launch * v_h^2
    // impact_at_time2: |- dist_target * grav_acc / 2 / inclination_launch == v_h^2
  }

  theory Slingshot_test{
    // h_launch = 1.0
    dist_target = 3.75 // == 15/4, because WolframAlpha said that this has a neat solution
    inclination_launch = 0.75
    //v_launch = 25/4
    include SlingshotScroll
  }

  // theory Result{
  //   val dist_target : float = 3.75
  //   val inclination_launch : float = 0.75
  //   include .Slingshot.Slingshot_groundLaunch
  //   val launch : .Slingshot.point
  //   val launch_base : .Slingshot.point = launch
  //   val h_launch : float = 0
  //   val h_launch_P : |- .Slingshot.dist(launch_base)(launch) == h_launch
  //   val v_z : float = (inclination_launch * v_h)
  //   val v_h : float = (((dist_target * grav_acc) / 2 / inclination_launch) ^ (1 / 2))
  //   val v_launch : float = (((v_z ^ 2) + (v_h ^ 2)) ^ (1 / 2))
  //   val v_launch_Ax : |- ((v_z ^ 2) + (v_h ^ 2)) == (v_launch ^ 2)
  //   val inclination_Ax : |- inclination_launch == (v_z / v_h)
  //   val target : .Slingshot.point
  //   val dist_target_P : |- .Slingshot.dist(launch_base)(target) == dist_target
  //   val grav_acc : float = 9.8100004196167
  //   val impact_time : float = (dist_target / (v_h))
  //   val impact_time_Ax : |- ((v_z + (((v_z * v_z) + (2 * h_launch * grav_acc)) ^ (1 / 2))) / grav_acc) == impact_time
  //   val impact_at_target_Ax : |- (v_h * impact_time) == dist_target
  //   val impact_time_Ax_Fixed : |- ((2 * v_z) / grav_acc) == impact_time
  //   val impact_at_time_Fixed : |- (dist_target * grav_acc) == (2 * inclination_launch * (v_h ^ 2))
  //   val impact_at_time_Fixed2 : |- ((dist_target * grav_acc) / 2 / inclination_launch) == (v_h ^ 2)
  // }
}